...
import { Button, Calendar, Card,Divider, Input, Progress, Space, Statistic, Table, Tag, theme } from "antd";
        ...
        connexionService.howIsLogIn()
            .then(res => {
                if (res.data.role === "COMMERCIAL") {
                    ...
                    TransactionService.getInfoTransaction(res.data.contactSim,dateFormatted)
                        .then((respd)=>{
                            setStat(respd.data)
                    ...
                    TransactionService.getStat(res.data.contactSim,lastWeekDay,dateFormatted)
                        .then(res => {
            ...
            .then(res => {
                if (res.data.role === "COMMERCIAL") {
                    ...
                    TransactionService.getTransactionByIntervenant(res.data.contactSim)
                        .then(resps=>{
                            //console.log("transaction:", resps.data);
        ...
        <div className='DashboardCommercial'>
            ...
            <div style={{margin:10, marginTop:17, display:'flex'}}>
                ...
                <p style={{margin:25, marginLeft:50, display:'flex'}}>
                    ...
                    <Statistic 
                        title={"Objectif "+commercial.objactifDate?"Objectif "+commercial.objactifDate:0} 
                        value={commercial.objectifMontant?commercial.objectifMontant:0+" XOF"} 
                        prefix={<CalendarOutlined />} 
                    />

                    <span style={{display:'block',margin:10}}></span>

                    <Statistic title="Date & Heure du dernier rechargement" 
                        value={commercial.dateDerniereRechargement?commercial.dateDerniereRechargement+" "+commercial.heureDerniereRechargement:0} 
                        prefix={<CalendarOutlined />} 
                    />
                </p>
            </div>
            <Divider orientation="left" dashed >Les 5 dernieres transactions</Divider>
            ...
            <Divider orientation="left" dashed >Performances</Divider>
            <Space>
                
                <Column {...configg} />
            </Space>
            <Divider orientation="left" dashed >Calendrier</Divider>
    ...
    return (
      <Card style={{ padding:0, margin:0 }}>
        <Space direction="horizontal">
          {icon}
          <Statistic title={title} value={value} valueStyle={{ color: 'rgb(94, 235, 51)', fontWeight:500 }}/>
        </Space>
      </Card>
    );
  }
